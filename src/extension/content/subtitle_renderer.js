/**
 * subtitle_renderer.js - Anchored Layout & Stable Subtitle Presentation.
 *
 * Guarantees the Anchored Layout Invariant:
 ...
 * - Dynamic runtime customization (font size, position offset, provisional opacity).
 */
...
export class SubtitleRenderer {
  ...
  applySettings(settings = {}) {
    ...
    if (settings.fontSize) {
      this.viewportEl.style.setProperty('--ylt-font-size', settings.fontSize);
    }
    if (settings.bottomOffset) {
      this.viewportEl.style.setProperty('--ylt-bottom-offset', `${settings.bottomOffset}px`);
    }
    if (typeof settings.provisionalOpacity === 'number') {
      this.viewportEl.style.setProperty('--ylt-provisional-opacity', settings.provisionalOpacity);
    }
  }
