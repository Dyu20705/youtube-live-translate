"""
protocol.py - Versioned wire protocol schemas and validators for Stage S5.
Version: 1.0
"""
...
def parse_and_validate_wire_message(raw_json: str) -> Dict[str, Any]:
    ...
    if msg_type == "subtitle.update":
        ...
        if len(str(data.get("committed_text", ""))) > 10000 or len(str(data.get("provisional_text", ""))) > 10000:
            raise ValueError("Text fields exceed maximum allowable length")
        if data.get("is_final") is not False and "is_final" in data:
        ...
        if data["segment_id"] < 0 or data["source_revision"] < 0:
            raise ValueError("segment_id and source_revision must be non-negative")
        if len(str(data.get("committed_text", ""))) > 10000:
            raise ValueError("committed_text exceeds maximum allowable length")


    elif msg_type == "status":
        if "state" not in data:
            raise ValueError("Missing required field 'state' for status message")
        valid_states = {"READY", "STARTING", "RUNNING", "DEGRADED", "STOPPED", "ERROR"}
    ...
    elif msg_type in ("control.start", "control.stop"):
        pass
